...
| P0-02 | P0 | Tracked production/demo source embeds a specific athlete ID, name and usernames in `js/shared/preprocessing/core.js`, `js/pages/swim/swim.js`, and `js/demo/generator.js`; the preprocessing path also numerically converts the provider identity | Violates the repository/Privacy Guide prohibition on identifiable athlete data in Git and the string-ID boundary; existing public history requires owner incident assessment even after code removal | Code repair is automatable; history/incident decision requires owner |
