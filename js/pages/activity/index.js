...
export function renderActivityPageError(
    documentObject = globalThis.document,
    historyObject = globalThis.history
) {
    if (!documentObject?.createElement) return;
    const host = documentObject.getElementById?.('activity-details')
        || documentObject.body;
...
async function defaultRenderer(input) {
    const { renderActivityPage } = await import('./activity.js');
    return renderActivityPage(input);
}
...
export async function initializeActivityPage({
    ...
    errorRenderer = renderActivityPageError
} = {}) {
...
if (typeof document !== 'undefined') {
    document.addEventListener('DOMContentLoaded', () => {
        if (!hasCoreVisualizationRuntime()) {
            renderActivityPageError();
            return;
        }
        void initializeActivityPage();
    }, { once: true });
}
